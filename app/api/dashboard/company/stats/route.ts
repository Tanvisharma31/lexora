...
export const GET = withAuth(
  async (context, request: NextRequest) => {
    try {
      // Check if user is IN_HOUSE_COUNSEL, COMPLIANCE_OFFICER, SUPER_ADMIN, or ADMIN
      ...
      const response = await fetch(`${BACKEND_URL}/dashboard/company/stats`, {
        method: "GET",
        headers,
      })
      ...
      if (!response.ok) {
        ...
        return NextResponse.json(
          { error: "Failed to fetch company dashboard stats", details: errorText },
      ...
      console.error("Company dashboard stats API error:", error)
      return NextResponse.json(
