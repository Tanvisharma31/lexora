...
export const GET = withAuth(
  async (context, request: NextRequest) => {
    try {
      // Check if user is LAWYER, ASSOCIATE, SUPER_ADMIN, or ADMIN
      ...
      const response = await fetch(`${BACKEND_URL}/dashboard/lawyer/stats`, {
        method: "GET",
        headers,
      })
      ...
      if (!response.ok) {
        ...
        return NextResponse.json(
          { error: "Failed to fetch lawyer dashboard stats", details: errorText },
      ...
      console.error("Lawyer dashboard stats API error:", error)
      return NextResponse.json(
