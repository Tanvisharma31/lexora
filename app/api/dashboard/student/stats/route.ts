...
export const GET = withAuth(
  async (context, request: NextRequest) => {
    try {
      // Check if user is STUDENT, SUPER_ADMIN, or ADMIN
      ...
      const response = await fetch(`${BACKEND_URL}/dashboard/student/stats`, {
        method: "GET",
        headers,
      })
      ...
      if (!response.ok) {
        ...
        return NextResponse.json(
          { error: "Failed to fetch student dashboard stats", details: errorText },
      ...
      console.error("Student dashboard stats API error:", error)
      return NextResponse.json(
